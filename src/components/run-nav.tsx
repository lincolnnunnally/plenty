export function RunNav() {
  return (
    <nav className="subnav" aria-label="Pantry desk">
      <a href="/run">Setup</a>
      <a href="/run/inventory">Inventory</a>
      <a href="/run/people">People</a>
      <a href="/run/shifts">Shifts</a>
      <a href="/run/pickups">Deliveries</a>
      <a href="/run/locations">Locations</a>
      <a href="/run/distributions">Distribution days</a>
      <a href="/run/donations">Gifts & assets</a>
      <a href="/run/stores">Store cards</a>
      <a href="/run/food">Food rescue</a>
      <a href="/run/around">Meet pantries</a>
      <a href="/run/receipts">Receipts</a>
      <a href="/run/promote">Promote</a>
    </nav>
  );
}
