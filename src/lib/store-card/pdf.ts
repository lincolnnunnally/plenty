...
export async function storeCardPdf(card: StoreCardPrint) {
  ...
  wrap(page, "Show this at the desk named on the card. The food is a gift. You do not have to buy anything else.", 40, 280, 530, 11, font, ink);
...
export async function holdListPdf(input: {
  ...
  let y = 700;
  y = wrap(page, `Hold donated food at ${input.holdDesk || "customer service"}. Families show a Plenty card. Extra purchase is not required.`, 40, y, 530, 12, fontBold, ink);
...
function drawWalletCard(
  ...
  wrap(page, `Show this at ${card.holdDesk || "customer service"}.`, x + 14, y + 88, 360, 11, fontBold, ink);
  const items = card.itemsText || "Ask for this week's Plenty hold.";
  wrap(page, items, x + 14, y + 68, 360, 10, font, ink);
  if (card.hoursText) wrap(page, card.hoursText, x + 14, y + 44, 360, 9, font, ink);
  wrap(page, "This food is a gift. You do not have to buy anything else.", x + 14, y + 18, 360, 9, fontBold, leaf);
